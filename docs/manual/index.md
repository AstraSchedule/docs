...
课程显示支持简称与全称两种模式。简称中可使用 `@` 下标语法（如 `自@物` 显示为"自<sub>物</sub>"），让课表更加清晰。系统提供三种高亮状态：已结束课程显示为灰色，当前课程根据 WebSocket 连接状态显示绿色或橙色，即将开始的课程以闪烁动画提示。通过托盘菜单中的"课上计时""上课隐藏""始终缩小"三个开关，可组合出四种显示模式，适应不同使用场景。倒计时框会跟踪当前高亮课程的位置平滑移动，以 MM:SS 格式显示剩余时间。
...
客户端托盘菜单提供二十余项功能，包括云端连接、更新管理、天气设置、课表操作、窗口模式、开机启动等。窗口在鼠标悬停时会自动透明，在需要临时关闭或者解决遮挡时很有用。自动更新机制支持自定义更新源和镜像代理。
